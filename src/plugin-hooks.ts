...
import {
	type RefreshActiveAccountQuotaResult,
	refreshActiveAccountQuota,
} from "./active-account-quota.js";
...
import {
	type ResolvedModelAccountCandidate,
...
function debugLog(message: string, details: Record<string, unknown>) {
	...
	appendFileSync(
		filePath,
		`[copilot-plugin-hooks debug] ${message} ${JSON.stringify(details)}\n`,
	);
}

function readRetryStoreContext(
	store: StoreFile | undefined,
): RetryStoreContext | undefined {
	if (!store) return undefined;

	const maybeLastAccountSwitchAt = (
		store as StoreFile & { lastAccountSwitchAt?: unknown }
	).lastAccountSwitchAt;
	return {
		networkRetryEnabled: store.networkRetryEnabled,
		lastAccountSwitchAt:
			typeof maybeLastAccountSwitchAt === "number"
				? maybeLastAccountSwitchAt
				: undefined,
		syntheticAgentInitiatorEnabled:
...
function mergeStoreWithCommonSettings(
	...
	return {
		...(store ?? { accounts: {} }),
		...(common?.networkRetryEnabled === true ||
		common?.networkRetryEnabled === false
			? { networkRetryEnabled: common.networkRetryEnabled }
			: {}),
		...(common?.experimentalSlashCommandsEnabled === true ||
		common?.experimentalSlashCommandsEnabled === false
			? {
					experimentalSlashCommandsEnabled:
						common.experimentalSlashCommandsEnabled,
				}
			: {}),
	};
}
...
async function readRequestModelID(
	request: Request | URL | string,
	init?: RequestInit,
) {
...
function rewriteRequestForAccount(
	request: Request | URL | string,
	enterpriseUrl?: string,
) {
	const nextBase = enterpriseUrl
...
function mergeAndRewriteRequestHeaders(
	...
	const hasRequestHeaders =
		request instanceof Request && [...request.headers].length > 0;
	const hasInitHeaders =
		init?.headers != null && [...new Headers(init.headers)].length > 0;
...
function getMergedRequestHeader(
	request: Request | URL | string,
	init: RequestInit | undefined,
	name: string,
) {
...
function getFinalSentRequestHeadersRecord(
	request: Request | URL | string,
	init: RequestInit | undefined,
) {
	const headers = new Headers(
	...
	return sanitizeLoggedRequestHeadersRecord(
...
function sanitizeLoggedRequestHeadersRecord(headers: Record<string, string>) {
	...
	if (
		...
		sanitized.authorization.length > 0
	) {
	...
	if (
		typeof sanitized["x-api-key"] === "string" &&
		sanitized["x-api-key"].length > 0
	) {
...
function getInternalSessionID(
	...
	const headerValue = getMergedRequestHeader(
		request,
		init,
	...
	const contextValue = (
		init as
			| (RequestInit & { [INTERNAL_SESSION_CONTEXT_KEY]?: unknown })
			| undefined
	)?.[INTERNAL_SESSION_CONTEXT_KEY];
...
function stripInternalSessionHeader(
	request: Request | URL | string,
	init?: RequestInit,
) {
...
function toReasonByInitiator(
	initiator: string | null,
): RouteDecisionEvent["reason"] {
...
function chooseCandidateAccount(input: {
	...
	const lowest = pickLowestWithRandom(
		input.candidates,
		input.loads,
		input.random,
	);
	...
	const bound =
		typeof boundName === "string"
			? input.candidates.find((item) => item.name === boundName)
			: undefined;
...
function pickLowestWithRandom(
	...
	const ranked = [...candidates].sort(
		(a, b) => (loads.get(a.name) ?? 0) - (loads.get(b.name) ?? 0),
	);
...
function pickLowestReplacementWithHardenedRandom(
	...
	const pickedIndex = Math.floor(
		toSafeReplacementRandomUnit(random) * tied.length,
	);
...
function normalizeStoreForRouting(store: StoreFile) {
	...
	for (const [modelID, candidate] of Object.entries(
...
export function buildPluginHooks(input: {
	...
	appendSessionTouchEventImpl?: (
		input: AppendSessionTouchEventInput,
	) => Promise<boolean>;
	appendRoutingEventImpl?: (input: {
		directory: string;
		event: RoutingEvent;
	}) => Promise<void>;
	appendRouteDecisionEventImpl?: (input: {
		directory: string;
		event: RouteDecisionEvent;
	}) => Promise<void>;
	readRoutingStateImpl?: (directory: string) => Promise<RoutingSnapshot>;
	triggerBillingCompensation?: (
		input: TriggerBillingCompensationInput,
	) => Promise<void>;
	...
	const authProvider =
		input.auth.provider ??
		COPILOT_PROVIDER_DESCRIPTOR.providerIDs[0] ??
		"github-copilot";
	const authLoaderMode =
		input.authLoaderMode ??
		(isCopilotProviderID(authProvider) ? "copilot" : "none");
	const enableCopilotAuthLoader = authLoaderMode === "copilot";
	const enableModelRouting =
		input.enableModelRouting ?? enableCopilotAuthLoader;
	...
	const refreshQuota =
		input.refreshQuota ??
		((store: StoreFile) => refreshActiveAccountQuota({ store }));
	const handleStatusCommandImpl =
		input.handleStatusCommandImpl ?? handleStatusCommand;
	const handleCompactCommandImpl =
		input.handleCompactCommandImpl ?? handleCompactCommand;
	const handleStopToolCommandImpl =
		input.handleStopToolCommandImpl ?? handleStopToolCommand;
	...
	const resolveOfficialChatHeaders: LoadOfficialChatHeaders =
		enableCopilotAuthLoader
	...
	const touchWriteCacheIdleTtlMs =
		input.touchWriteCacheIdleTtlMs ?? TOUCH_WRITE_CACHE_IDLE_TTL_MS;
	const touchWriteCacheMaxEntries =
		input.touchWriteCacheMaxEntries ?? MAX_TOUCH_WRITE_CACHE_ENTRIES;
	const appendSessionTouchEventImpl =
		input.appendSessionTouchEventImpl ?? appendSessionTouchEvent;
	const appendRoutingEventImpl =
		input.appendRoutingEventImpl ?? appendRoutingEvent;
	const appendRouteDecisionEventImpl =
		input.appendRouteDecisionEventImpl ?? appendRouteDecisionEvent;
	const readRoutingStateImpl = input.readRoutingStateImpl ?? readRoutingState;
	const triggerBillingCompensation =
		input.triggerBillingCompensation ?? (async () => {});

	const loadCandidateAccountLoads =
		input.loadCandidateAccountLoads ??
		(async (ctx: { candidates: ResolvedModelAccountCandidate[] }) => {
	...
	const loadMergedStoreSync = () =>
		mergeStoreWithCommonSettings(loadStoreSync(), loadCommonSettingsSync?.());
	...
	const isNetworkRetryEnabled = async (retryStore?: RetryStoreContext) => {
		...
		const store = readRetryStoreContext(
	...
	const classifyRequestReason = async (requestInput: {
		...
		const initiator = getMergedRequestHeader(
		...
		const sessionLookup = sessionClient?.get as
			| undefined
			| ((input: {
					path: { id: string };
		...
		const messageLookup = sessionClient?.message as
			| undefined
			| ((input: {
		...
		const messageIDHeader = getMergedRequestHeader(
			requestInput.request,
			requestInput.init,
			INTERNAL_DEBUG_LINK_HEADER,
		);
		if (typeof messageIDHeader === "string" && messageIDHeader.length > 0) {
			const currentMessage = await messageLookup
				...
				.catch(() => undefined);
			const parts = Array.isArray(currentMessage?.data?.parts)
				? currentMessage.data.parts
				: undefined;
		...
		const isTrueChildSession =
			typeof session?.data?.parentID === "string" &&
			session.data.parentID.length > 0;
		if (
			canDetermineSessionAncestry &&
			!isTrueChildSession &&
			requestInput.hasExistingBinding === false
		) {
			return {
	...
	const clearAccountSwitchContext =
		input.clearAccountSwitchContext ??
		(async (capturedLastAccountSwitchAt?: number) => {
			...
			try {
				...
				console.warn(
					"[plugin-hooks] failed to clear account-switch context",
					error,
				);
			}
		});
	...
	const loader: AuthLoader = async (getAuth, provider) => {
		...
		const finalHeaderCapture = new AsyncLocalStorage<
			((headers: Record<string, string>) => void) | undefined
		>();
		const getScopedAuth = async () => authOverride.getStore() ?? getAuth();
		const providerConfig = provider as unknown as
			...
			...(input.loadOfficialConfig == null
				? {
						baseFetch: async (nextRequest, nextInit) => {
							finalHeaderCapture.getStore()?.(
								getFinalSentRequestHeadersRecord(nextRequest, nextInit),
							);
		...
		const finalizeRequestForSelection =
			input.finalizeRequestForSelection ??
			(input.loadOfficialConfig
				...
				: async (selectionInput: {
						...
						const captureConfig = await loadOfficialConfigForCopilot({
							getAuth: getScopedAuth as () => Promise<
								CopilotAuthState | undefined
							>,
							provider: providerConfig,
							baseFetch: async (nextRequest, nextInit) => {
								captured = {
						...
						const inspectionRequest =
							selectionInput.request instanceof Request
								? selectionInput.request.clone()
								: selectionInput.request;
						await captureConfig
							.fetch(inspectionRequest, selectionInput.init)
							.catch(() => undefined);
						return captured;
					});
		...
		const fetchWithModelAccount = async (
			request: Request | URL | string,
			init?: RequestInit,
		) => {
			const latestStoreRaw = await loadStore().catch(() => undefined);
			const latestStore = latestStoreRaw
				? normalizeStoreForRouting(latestStoreRaw)
				: undefined;
			...
			const candidates = latestStore
				? resolveCopilotModelAccounts(latestStore, modelID)
				: [];
			...
			const hasExistingBinding =
				sessionID.length > 0 && sessionAccountBindings.has(sessionID);
			...
			const selectionAllowReselect =
				classification.reason === "user-reselect" ||
			...
			const hasUsableExplicitModelCandidate = candidates.some(
				(item) => item.source === "model",
			);
			if (hasExplicitModelGroup && !hasUsableExplicitModelCandidate) {
				throw new Error(`No usable account for model ${modelID}`);
			}
			const loads =
				latestStore && candidates.length > 0
					? toLoadMap(
							await loadCandidateAccountLoads({
								sessionID,
								modelID,
								store: latestStore,
								candidates,
			...
			let decisionTouchWriteOutcome: RouteDecisionEvent["touchWriteOutcome"] =
				"skipped-missing-session";
			let decisionTouchWriteError: string | undefined;
			let finalChosenAccount = resolved.name;
			let chosenAccountAuthFingerprint = toAuthFingerprint(
				resolved.entry.refresh,
			);
			let finalRequestHeaders = getFinalSentRequestHeadersRecord(
				selectionRequest,
				selectionInit,
			);
			...
			const previousBindingAccount =
				sessionID.length > 0
					? sessionAccountBindings.get(sessionID)?.accountName
					: undefined;
			const debugLinkId =
				getMergedRequestHeader(
					selectionRequest,
					selectionInit,
					INTERNAL_DEBUG_LINK_HEADER,
				) ?? undefined;
			...
			const shouldStripAgentInitiator =
				classification.reason === "unbound-fallback" ||
			...
			const sendBillingCompensationIfNeeded = async (input: {
				...
				if (
					typeof previousBindingAccount !== "string" ||
					previousBindingAccount.length === 0
				)
					return;
			...
			const sendWithAccount = async (
				candidate: ResolvedModelAccountCandidate,
				requestValue: Request | URL | string,
				initValue: RequestInit | undefined,
			) => {
				...
				const deduplicated = stripDuplicateHeadersFromRequestWhenInitOverrides(
					requestValue,
					initValue,
				);
				const outbound = stripInternalSessionHeader(
				...
				return finalHeaderCapture.run(
					...
					() =>
						authOverride.run(candidateAuth, () =>
							config.fetch(
								rewriteRequestForAccount(
									outbound.request,
									candidate.entry.enterpriseUrl,
								),
								outbound.init,
							),
						),
			...
			let rateLimitEvidence: { matched: boolean; retryAfterMs?: number } = {
				matched: false,
			};
			try {
			...
			if (rateLimitEvidence.matched) {
				...
				if (queue.length >= RATE_LIMIT_HIT_THRESHOLD) {
					...
					if (routingSnapshot) {
						...
						const currentLoad =
							nextLoads.get(resolved.name) ?? loads.get(resolved.name) ?? 0;
						...
						const replacements = cooledCandidates.filter(
							(item) => (nextLoads.get(item.name) ?? 0) <= currentLoad,
						);
						const replacement = pickLowestReplacementWithHardenedRandom(
							replacements,
							nextLoads,
							random,
						);
						...
						if (replacement) {
							...
							if (typeof rawPayload === "string") {
								try {
									const parsed = JSON.parse(rawPayload) as Record<
										string,
										unknown
									>;
									const cleaned = await cleanupLongIdsForAccountSwitch({
										payload: parsed,
									});
									if (cleaned.changed) {
										const rewritten = mergeAndRewriteRequestHeaders(
							...
							finalRequestHeaders = getFinalSentRequestHeadersRecord(
								retriedRequest,
								retriedInit,
							);
							...
							chosenAccountAuthFingerprint = toAuthFingerprint(
								replacement.entry.refresh,
							);
		...
		return {
			...
			fetch: createRetryFetch(providerFetch, {
				...
				clearAccountSwitchContext: async () =>
					clearAccountSwitchContext(retryStore?.lastAccountSwitchAt),
			}),
		};
	};
	...
	const chatHeaders: ChatHeadersHook = async (hookInput, output) => {
		if (
			!enableCopilotAuthLoader ||
			!isCopilotProviderID(hookInput.model.providerID)
		)
			return;
		...
		const officialWroteInitiator =
			initiatorAfterOfficial !== initiatorBeforeOfficial;
		...
		if (shouldCheckSyntheticInitiator) {
			const currentMessage = await (
				input.client?.session?.message as
					| undefined
					| ((input: {
							path: {
								id: string;
			...
			const currentParts = Array.isArray(currentMessage?.data?.parts)
				? (currentMessage.data.parts as Array<DebugPart>)
				: undefined;
			const hasSyntheticTextPart =
				currentParts?.some(
		...
		if (isDebugEnabled()) {
			const currentMessage = await input.client?.session
				?.message?.({
					path: {
						id: hookInput.message.sessionID ?? hookInput.sessionID,
			...
			const session = await input.client?.session
				...
				.catch(() => undefined);
			const recentMessages = await input.client?.session
				?.messages?.({
					path: {
						id: hookInput.sessionID,
					},
			...
			const messageList = (recentMessages?.data ??
				[]) as Array<DebugSessionMessage>;
			...
			debugLog("chat.headers evidence", {
				evidence: {
					...
					recent_messages: messageList.slice(0, 4).map((message) => ({
						...
						part_types: toPartTypeList(
							message.parts as Array<DebugPart> | undefined,
						),
					})),
				...
				candidates: {
					synthetic_text_count: currentTextParts.filter((part) =>
						Boolean(part.synthetic),
					).length,
					matches_continue_template: currentTextParts.some((part) =>
						/continue/i.test(part.preview),
					),
	...
	return {
		...
		} as AuthProvider extends never
			? never
			: NonNullable<CopilotPluginHooks["auth"]>,
		config: async (config) => {
			...
			if (enableCopilotAuthLoader) {
				config.command["copilot-status"] = {
					template:
						"Show the current GitHub Copilot quota status via the experimental workaround path.",
					description: "Experimental Copilot quota status workaround",
				};
				config.command["copilot-compact"] = {
					template:
						"Summarize the current session via real session compacting flow.",
					description: "Experimental compact command for Copilot sessions",
				};
				config.command["copilot-stop-tool"] = {
					template:
						"Interrupt the current session tool flow, annotate the interrupted result, and append a synthetic continue.",
					description:
						"Experimental interrupt-and-annotate recovery with synthetic continue for Copilot sessions",
				};
			}
		},
		"command.execute.before": async (hookInput) => {
			...
			if (hookInput.command === "copilot-stop-tool") {
				...
				await handleStopToolCommandImpl({
					client: input.client ?? {},
					sessionID: hookInput.sessionID,
					runningTools: (hookInput as { runningTools?: unknown[] })
						.runningTools,
					syntheticAgentInitiatorEnabled:
						store?.syntheticAgentInitiatorEnabled === true,
				});
